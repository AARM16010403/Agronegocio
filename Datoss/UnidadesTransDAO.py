...
class UnidadesTransDAO:
    ...
    def insertarUnidad(self,unidad):
        sql = 'insert Ventas.UnidadesTransporte (idUnidadTransporte,placas,marca,modelo,anio,capacidad) values (?,?,?,?,?,?)'
        try:
            Values = [unidad.idUnidadTransporte,unidad.placas,unidad.marca,unidad.modelo,unidad.anio,unidad.capacidad]
            cursor=self.db.cursor()
    ...
    def consultaIndividual(self,id):
        ...
        try:
            ...
            cursor.execute(sql,Values)
            rs = cursor.fetchone()
            u = UnidadTrans(rs[0], rs[1], rs[2], rs[3],rs[4],rs[5])
            cursor.close()
            self.db.close()
        except pyodbc.Error as e:
            print(e)
