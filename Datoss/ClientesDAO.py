...
class ClientesDAO:
    ...
    def obtenerClientes(self):
        sql = 'select idCliente,nombre,razonSocial,limiteCredito,direccion,' \
              'codigoPostal,rfc,telefono,email,tipo,idCiudad from Ventas.Clientes'
        lista = []
        try:
    ...
    def insertarCliente(self,cliente):
        sql = 'insert Ventas.Clientes (idCliente,nombre,razonSocial,limiteCredito,direccion,codigoPostal,' \
              'rfc,telefono,email,tipo,idCiudad) values (?,?,?,?,?,?,?,?,?,?,?)'
        try:
    ...
    def consultaIndividual(self,id):
        ...
        try:
            ...
            cursor.execute(sql,Values)
            rs = cursor.fetchone()
            c = Cliente(rs[0], rs[1], rs[2], rs[3],rs[4],rs[5],rs[6],rs[7],rs[8],rs[9],rs[10])
            cursor.close()
            self.db.close()
        except pyodbc.Error as e:
            print(e)
